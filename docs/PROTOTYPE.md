...
A validated IndexedDB record stores Favorites, Library songs, albums, and playlists with separate add timestamps where applicable, queue references, current queue position, repeat mode, the global preferred recording type, and per-song recording preferences. Legacy favorite IDs and previously combined saved songs migrate into both Favorites and Library Songs automatically. On reload the client resolves valid queue references against the current revisioned catalog and restores the selected track paused, preserving browser autoplay expectations. The prior `livingMusic:userState:v1` record migrates transactionally and is removed only after verification. Malformed storage, unavailable storage, removed songs, and removed recordings degrade to valid defaults; the dedicated Settings page provides appearance and installation controls, storage status, JSON backup/restore, download removal, and clear-data controls.


## Step 7 behavior

Supported browsers receive Media Session metadata for the active title, artist, collection, and artwork. Lock-screen, headset, keyboard, and operating-system controls can play, pause, move between tracks, and seek on the timeline. Previous- and next-track actions take priority over interval-seek actions so iOS can present song navigation in its limited transport slots. Buffering remains an active Media Session state, and external Play actions recover stalled or failed sources at their last known position. The integration treats every Media Session feature as optional so browsers with partial or no support retain the complete in-page player. Space toggles playback when keyboard focus is outside a link or form control.

The app adds higher-contrast and forced-colors adaptations, complete sharing and install metadata, and dedicated release notes. A generated service worker precaches content-addressed app-shell assets, reuses unchanged files across releases, and leaves catalog and media requests on their existing network paths. The retired static prototype has been removed, leaving Vite's source and `dist/` as the only application paths. Automated tests cover catalog validation, audio selection, queue behavior, storage migration, routing, search, and Media Session actions. Production QA checks the responsive 390 px layout, desktop layout, live API, local persistence, and the minified Pages build.


## Post-prototype PWA behavior

The completed PWA phases add atomic shell and catalog caching, controlled updates, IndexedDB listener data with backup/restore, contextual installation, and explicit offline music. Song menus and album or playlist headers can download selected recordings; Downloaded in Library lists saved music. The service worker resolves downloaded audio and video from a dedicated cache, synthesizes byte ranges for readable responses, and retains opaque responses for hosts without CORS. Download state survives restarts, interrupted work becomes retryable, quota errors leave listener data intact, and catalog URL changes produce an updateable stale state. Removing downloaded bytes never removes the song from Library, Favorites, or playlists.
