...
Living Music publishes play, pause, previous-track, next-track, and timeline-seek actions through the Media Session API. On supported iOS versions, the previous and next actions appear in Now Playing on the Lock Screen and in Control Center. Interval seek actions are intentionally omitted so iOS can use those transport positions for song navigation; listeners can still scrub with the system timeline.
...
The service worker maintains a separate `living-music-catalog-v2` runtime cache. `/musicapi/index.json` uses network-first loading with the last complete manifest as fallback; the multilingual root plus its default language index and search payload must be available before that fallback is replaced. Revisioned catalog indexes, search data, and opened collections use cache-first loading, retaining the two newest responses for each logical path. The interface identifies offline, saved-catalog, upstream, unsupported-schema, and malformed-data states and retries the catalog automatically when connectivity returns. Church-hosted audio remains outside automatic caching. Viewed artwork uses a separate cache-first store capped at 512 entries, enough to retain the complete album grid plus recently viewed song artwork. The first 12 album covers receive eager, high-priority loading so visible Browse content fills promptly.

## Installation and local data

The gear beside the sidebar’s project links opens a dedicated Settings page; the mobile header provides the same shortcut. Settings contains appearance, privacy, installation guidance, local storage controls, and the semantic version plus commit-derived build ID. Browsers with an install prompt provide a direct **Install** action; iPhone and iPad show Safari’s Share → Add to Home Screen instructions. Installed windows report the current installation state. The manifest launches into Browse and includes shortcuts for Browse, Search, Favorites, and Playlists.

Production supports Google Analytics 4 for sanitized page-view counts. A first-run banner remains visible until the listener allows or declines analytics, and the Google tag is not loaded before permission is granted. Dynamic collection and playlist identifiers are removed before reporting, and the app does not report song titles, search text, playlist names, or Library contents. Advertising storage, signals, and personalization remain disabled. The Settings privacy switch can change the choice later. Development and test builds do not load the Google tag.
...
Production loads the same-origin catalog from `/musicapi/`. Local development defaults to the published API. The client first requests `index.json`, requires its advertised multilingual schema v2 root, and resolves language, collection, and search links against the document that declared them. Browse remembers the selected catalog language on the current device. English retains the complete catalog; translated languages contain only tracks explicitly published for that language.

The client qualifies collection and song identities with their language before they enter Library, playlists, downloads, or the playback queue. Catalog payloads are untrusted network input. The client validates collection summaries, songs, and recordings before rendering them, and all catalog strings are rendered as text. Null artwork fields may be omitted by the compact API; the interface provides a local fallback. Collection payloads are fetched only when their revisioned hash route is opened.
