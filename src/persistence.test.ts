...
describe("listener data backups", () => {
  it("round trips listener state through a versioned backup", () => {
    const state = { ...EMPTY_USER_STATE, favorites: ["eng::song:one"], librarySongs: ["eng::song:one"] };
    ...
    expect(importUserState(backup)).toMatchObject({ favorites: ["eng::song:one"], librarySongs: ["eng::song:one"] });
  });
